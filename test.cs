...
//                            {
//                                #region
//                                ftp.CreateDirectory(direc_upload, true); //上传文件       
//                                bool res = ftp.UploadFile(direc_pdf + guid + ".pdf", @"\" + direc_upload + @"\" + guid + ".pdf"); //如果上传成功!插入数据库记录                         
//                                if (res)
//                                {
...
//                                        }
//                                    }
//                                    sql = @"insert into list_attachment (ID,FILEPATH,FILENAME,FILESIZE,ORDERCODE,CREATETIME,CREATEUSERID,ISUPLOAD) 
//                                    VALUES (LIST_ATTACHMENT_ID.NEXTVAL,'{0}','{1}','{2}','{3}',sysdate,'{4}','1')";
...
//                                    }
//                                    #region 如果需要生成关联单文件
//                                    if (this.checkBox1.Checked && !string.IsNullOrEmpty(barcode))
//                                    {
...
//                                            sql = @"insert into list_attachment (ID,FILEPATH,FILENAME,FILESIZE,ORDERCODE,CREATETIME,CREATEUSERID,ISUPLOAD)
//                                            VALUES (LIST_ATTACHMENT_ID.NEXTVAL,'{0}','{1}','{2}','{3}',sysdate,'{4}','1')";
//                                            sql = string.Format(sql, @"/" + direc_upload + @"/" + guid2 + ".pdf", guid2 + ".pdf", file.Length, newcode, this.label1.Text);
//                                            com = new OracleCommand(sql, conn);
//                                            com.ExecuteNonQuery();
